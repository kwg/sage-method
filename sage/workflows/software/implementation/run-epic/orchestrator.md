# Epic Orchestrator (Software Module - Phased Execution Loop)

```xml
<critical>VERSION 1.0: Phased execution with persistent state</critical>
...
<overview>
  ...
  - Mandatory smoke tests before finalization
  - Feature branch naming validation
  - Learning recorder for failure analysis
</overview>

<components>
  This orchestrator uses core SAGE components:
...
<phase-transitions>
  Normal flow (Software Module):

  00-init → 01-story-start
  01-story-start → 02-plan (or 07-epic-qa if no more stories)
  02-plan → 03-implement
  03-implement → 03-implement (more chunks) | 04-test (all chunks done)
  04-test → 05-review (tests pass) | 04-test (retry with FIXER)
  05-review → 05-review (issues found, retry) | 06-story-complete (clean)
  06-story-complete → 01-story-start (more stories) | 07-epic-qa (all done)
  07-epic-qa → 08-finalize (tests pass) | 07-epic-qa (retry)
  08-finalize → done
  ...
  - Learning recorder captures failure context
</phase-transitions>
...
<git-safety>
  ...
  - Story branch: feature/{story_id}  (validated in phase 01)
...
```
User: *run-epic
Agent: Found existing state for epic-EPIC-002 at phase 03-implement.
...
Agent: [Reads and displays state/epic-EPIC-002-state.json]
```

### Learning Summary

```
User: What patterns have we seen?
Agent: [Queries learning recorder, displays failure patterns]
```
