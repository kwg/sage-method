...
Will update existing story file rather than creating new one.
  ...
  <step n="6" goal="Assemble acceptance criteria and tasks">
    ...
    <!-- EMBEDDED DOCUMENTATION PRINCIPLE -->
    <note>Each agent documents their own domain as part of task completion. No dedicated Tech Writer - the implementer captures context. Stories should be structured so the executing agent can document their translation decisions, patterns discovered, and implementation rationale directly in the "Workflow Documentation" section of Dev Agent Record.</note>

    <action>For UI/UX stories involving Pixel:
      - Pixel owns the full translation from design to Godot properties
      - Story should specify design intent AND exact Godot implementation details
      - Link receives implementation-ready tasks (anchors, margins, sizes) not pixel coordinates
      - Link executes and validates, raising objections only for technical blockers
    </action>
  ...
  <step n="6.5" goal="Design agent tests for visual verification">
    <critical>Agent tests enable autonomous QA verification after implementation. Reference docs/workflows/agent-testing-workflow.md for schema.</critical>

    <action>For each acceptance criterion, determine if it has a visually verifiable outcome:
      - UI changes → screenshot verification
      - State transitions → game state command verification
      - Navigation → scene type verification
      - Data persistence → state query verification
    </action>

    <action>Design 1-3 agent tests per story based on scope:
      - **test_name**: snake_case identifier matching the AC being tested
      - **Description**: What the test verifies (maps to AC)
      - **Scene**: Starting scene (main_menu, combat, map, shop, rest, event, rewards, settings)
      - **Commands**: Sequence to reach testable state (use available commands from agent-testing-workflow.md)
      - **Verify**: Expected state output patterns or screenshot checkpoints
    </action>

    <action>Common command patterns by story type:
      - **Combat stories**: game goto combat → game state → game play_card/end_turn → screenshot
      - **Map/navigation stories**: game goto map → game map_state → game select_node → game scene
      - **UI stories**: game goto [scene] → game ui_positions → game click_button → screenshot
      - **Save/persistence stories**: game goto [scene] → [actions] → quit → relaunch → game state
    </action>

    <action>Always include baseline verification:
      - `No SCRIPT ERROR` in verify section
      - Screenshot at key checkpoint for visual evidence
    </action>

    <template-output file="{default_output_file}">agent_tests</template-output>
  </step>
  ...
  <step n="7" goal="Create or update story document">
    ...
    <action>PATTERN INJECTION (Tell Don't Ask Policy):
      - Load patterns from {patterns_registry_content} (from docs/patterns/*.md)
      - Review the Pattern Applicability table to identify which patterns apply to this story
      - Consider story scope: Does it involve FSM/phases? Signal handlers? Async operations?
      - Add "Required Patterns" subsection to Dev Notes with applicable patterns
      - Include code examples from the registry for each pattern
      - Format example:
        ```
        ### Required Patterns

        This story involves [async signal handling / FSM transitions / etc.]. Apply these patterns:

        **P1: Deferred Phase Transition** - Use `call_deferred()` for phase transitions triggered by signals
        ```gdscript
        # Example from pattern registry
        func _enter_phase() -> void:
            EventBus.phase_started.emit()
            call_deferred("_deferred_transition_to_next")
        ```

        **P4: Async Null Guards** - Validate state at start of signal handlers
        ```gdscript
        ...
        [Source: docs/patterns/async-signal-patterns.md]
        ```
    </action>
  ...
  <step n="8" goal="Validate, save, and mark story drafted" tag="sprint-status">
    <invoke-task>Validate against checklist at {installed_path}/checklist.md using {project-root}/sage/core/tasks/validate-workflow.xml</invoke-task>
