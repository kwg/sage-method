# Create Epic - Core Workflow Instructions

<critical>The workflow execution engine is governed by: {project-root}/sage/core/tasks/workflow.xml</critical>
<critical>You MUST have already loaded and processed: {project-root}/sage/workflows/core/epic/create-epic/workflow.yaml</critical>
...
WORKFLOW PHASES:
1. Retrospective Review & Epic Validation (with state file learning extraction)
...
This workflow will:
1. Review ALL previous retrospectives AND state files to extract learnings
...
<action>Validate epic exists in {sprint_status_file} or epic definition files</action>

<check if="epic not found">
  <output>
**Error:** Epic "{{epic_identifier}}" not found in sprint-status or epic definitions.
...
<action>Load epic definition from epic source files</action>
...
<!-- NEW: Load epic state files first -->
<action>Load epic state files from .sage/state/epic-*-state.json</action>

<check if="state files found">
  <output>
Found {{state_file_count}} epic state file(s). Extracting learning records...
  </output>

  <action>For each state file:
    - Extract learning_records[] array
    - Extract retro_notes{} object
    - Parse patterns identified during execution
    - Extract failure cascades and their resolutions
    - Merge with retrospective markdown findings
  </action>

  <output>
**State File Learning Records:**

| Epic | Learning Records | Retro Notes | Patterns |
|------|------------------|-------------|----------|
{{state_file_summary_table}}
  </output>
</check>

<check if="no state files found">
  <output>
**Note:** No epic state files found. Learning extraction will use retrospective markdown only.
  </output>
</check>
...
Found {{retro_count}} retrospective(s). Extracting learnings...
  ...
  **Process Improvements:**
  ...
  <action>Aggregate learnings across all retros AND state files</action>
...
**Learning Records from State Files:** {{learning_record_count}}
  </output>
</check>

<action>Load patterns from {patterns_dir}</action>
<action>Match patterns to stories based on keywords and state file insights</action>
...
<action>Cross-reference epic stories against retro learnings AND state file records</action>
...
- Which learning records from state files inform it
...
- Stories informed by state file learnings: {{stories_with_state_learnings}}
</output>
...
## Story Definition
{{story_definition_from_epic_source}}
...
{{relevant_learning_records_from_state_files}}
...
- Include learnings from predecessor stories and state files
...
## State File Learnings Applied

{{state_file_learnings_applied}}
...
<error-handling>
  ...
  <on-missing-state-files>
    - Note that state file learnings are unavailable
    - Continue with retrospective markdown only
    - Recommend running run-epic-orchestrated for future epics
  </on-missing-state-files>

  <on-missing-epic-definition>
