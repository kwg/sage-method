...
<critical>This workflow handles BOTH new projects AND legacy projects following the SAGE Method</critical>

<workflow>

<step n="1" goal="Scan for existing work">
<output>Welcome to SAGE Method, {user_name}!</output>
...
To check progress: Load any BMM agent and run /sage:bmm:workflows:workflow-status
...
1. **SAGE Method** - Full planning for complex projects
...
<output>Based on your project, here are your SAGE Method planning options:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1. SAGE Method** 🎯 {{#if recommended}}(RECOMMENDED){{/if}}
...
<ask>Which SAGE Method approach fits best?
...
<ask if="n">Which SAGE Method track instead?
...
<output>Your SAGE workflow path:
...
**Agent:** {{next_agent}}
**Command:** /sage:bmm:workflows:{{next_workflow_id}}

{{#if next_agent not in [analyst, pm]}}
💡 Start new chat with **{{next_agent}}** agent first.
{{/if}}

To check progress: /sage:bmm:workflows:workflow-status
