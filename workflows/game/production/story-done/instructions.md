...
<step n="2.5" goal="Pattern Discovery and Propagation">
<critical>PATTERN DISCOVERY: Check for new patterns discovered during implementation and propagate to pending stories</critical>

<action>Review the completed story's Dev Notes and Completion Notes sections for:
  - New async/signal handling patterns discovered
  - Race condition fixes applied
  - State management solutions created
  - Any "gotcha" or workaround that future stories should know about
</action>

<action>Check if any new patterns were discovered that aren't in the patterns registry:
  - Load {output_folder}/patterns/async-signal-patterns.md
  - Compare story learnings against Pattern Index table
  - Identify any new patterns not already documented
</action>

<check if="new patterns discovered">
  <action>Update patterns registry ({output_folder}/patterns/async-signal-patterns.md):
    - Add new pattern with unique ID (P7, P8, etc.)
    - Document: Problem, Solution, When to apply, Code example
    - Add to Pattern Index table with story reference
    - Update Pattern Applicability by Epic table if needed
  </action>

  <action>Find remaining stories that need pattern injection:
    - Load {sprint_status}
    - Find all stories with status "drafted" or "ready-for-dev"
    - These stories were drafted before this pattern was discovered
  </action>

  <check if="pending stories exist">
    <action>For each pending story:
      - Load story file from {story_dir}
      - Find "Dev Notes" or "Required Patterns" section
      - Add new pattern to the Required Patterns subsection
      - Include code example from registry
      - Add note: "[Pattern added post-draft from Story {story_key}]"
      - Save updated story file
    </action>

    <output>📝 Pattern propagation complete:
- New pattern(s) added to registry
- Updated {pending_story_count} pending stories with new pattern(s)
    </output>
  </check>

  <check if="no pending stories">
    <output>📝 Pattern added to registry (no pending stories to update)</output>
  </check>
</check>

<check if="no new patterns discovered">
  <action>Skip pattern propagation - no updates needed</action>
</check>
